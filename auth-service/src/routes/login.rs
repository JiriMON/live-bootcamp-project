...
#[tracing::instrument(name = "Handle 2 factor", skip_all)]
async fn handle_2fa(
    email: &Email,
    state: &AppState, 
    jar: CookieJar,
    ...
    println!("{:#?}{:#?}{:#?}",email.clone(),login_attempt_id.clone(),two_fa_code.clone());
    if let Err(e) = state
        .two_fa_code_store
        .write()
        .await
...
#[tracing::instrument(name = "Handle no 2 factor", skip_all)]
