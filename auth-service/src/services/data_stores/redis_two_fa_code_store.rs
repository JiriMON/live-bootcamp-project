...
use crate::domain::{
    data_stores::{TwoFACodeStore, TwoFACodeStoreError}, Email, LoginAttemptId, TwoFACode,
};
...
impl TwoFACodeStore for RedisTwoFACodeStore {
    async fn add_code(
        ...
        //let value: (LoginAttemptId, TwoFACode) = (login_attempt_id,code);
        let value_json = to_string(&(login_attempt_id.as_ref().to_string(),code.as_ref().to_string())).map_err(|_| TwoFACodeStoreError::UnexpectedError)?;
        //println!("{}", key.clone());
    

        let _: () = self.conn.write().await.set_ex(key,&value_json,TEN_MINUTES_IN_SECONDS).map_err(|_| TwoFACodeStoreError::UnexpectedError)?;
        Ok(())
    ...
    async fn remove_code(&mut self, email: &Email) -> Result<(), TwoFACodeStoreError> {
        ...
        let key = get_key(email);
        let _:() = self.conn.write().await.del(&key).map_err(|_| TwoFACodeStoreError::UnexpectedError)?;
